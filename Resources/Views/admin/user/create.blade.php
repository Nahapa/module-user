...
<section class="dash_content_app">
    ...
    <header class="dash_content_app_header">
        <h2 class="icon-plus">Novo Usuário</h2>
        ...
        <div class="dash_content_app_header_actions">
            <nav class="dash_content_app_breadcrumb">
                <ul>
                    ...
                    <li class="separator icon-angle-right icon-notext"></li>
                    <li><a href="{{ route('admin.user.create') }}" class="text-primary">Novo Usuário</a></li>
                </ul>
            </nav>
        </div>
    </header>
    ...
    <div class="dash_content_app_box">
        <div class="nav">
            ...
            <form class="app_form" action="{{ route('admin.user.store') }}" method="post" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="app_collapse mt-2">
                        ...
                        <div class="app_collapse_content d-none">
                            @foreach($roles as $role)
                                <div class="label">
                                    <label class="label">
                                        <input type="checkbox" name="{{ $role->id }}" {{ old($role->id) ? 'checked' : '' }}><span>{{ $role->name }}</span>
                                    </label>
                                </div>
                            @endforeach
                        </div>
